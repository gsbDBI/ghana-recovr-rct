...
ggplot(dat, aes(x=dem1, fill = dem2)) + 
  ...
  annotate(geom = 'text', 
           ...
           label = filter(mean_age, dem2 == 'Female')$age, y=40, 
           color = gg_color_hue(2)[1]) +
  annotate(geom = 'text', 
           x = filter(mean_age, dem2 == 'Male')$age+3, 
           label = filter(mean_age, dem2 == 'Male')$age, y=45, 
...
# Household size
ggplot(dat, aes(x=dem9)) + 
  geom_histogram() + 
  geom_vline(aes(xintercept = round(mean(dat$dem9), 2)), 
             linetype='dashed') +
  annotate(geom = 'text', 
           x = round(mean(dat$dem9), 2)+2, 
           label = round(mean(dat$dem9), 2), y=170,) +
  xlab('Household Size')
...
cov5 <- c(rep('Manufac. \n medical', sum(dat$cov5_1)),
          ...
          rep('Refuse to answer', sum(dat$cov5__888)))
